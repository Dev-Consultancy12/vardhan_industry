...
PACKING_SLIP_PATH = "sample_inputs/Packing Slip.xlsx"
ITEM_CODES_TRACKER_PATH = "sample_inputs/Item Codes Tracker.xlsx"
TEST_READINGS_PATH = "sample_inputs/TEST READINGS.xlsx"
OUTPUT_DIR = os.path.join(BASE_DIR, "v2output")
...
try:
    ...
    print(f"Error reading Group Names: {e}")
    group_names_list = []
...
try:
    ...
    print(f"Error reading Item Codes Tracker: {e}")
    exit(1)
...
for idx, row in readings_df.iterrows():
    c1 = str(row.get(1, '')).strip()
    if c1 and c1 not in ["nan", "None"]:
        param_rows.append((c1, idx))

param_rows.append(("END", len(readings_df)))

pools = {}
for i in range(len(param_rows) - 1):
    param_name = param_rows[i][0]
    start_row = param_rows[i][1]
    end_row = param_rows[i+1][1] - 1
    
    vals = []
    for r in range(start_row, end_row + 1):
        v = readings_df.iloc[r, 3] # Column D
        if pd.notna(v):
            try:
                vals.append(float(v))
            except:
                pass
    if vals:
        pools[param_name] = vals

def generate_obs(param_name, fixed, copper_type, spec, count, item_colour=""):
    if param_name == "Colour":
        return [item_colour] * count
    if param_name == "No of strands":
        num_str = "".join([c for c in spec if c.isdigit()])
        num = int(num_str) if num_str else 20
        return [num] * count
    if param_name == "Printing over the cable":
        return ['NA'] * count
    if param_name == "Volume Resistivity":
        return ['2X10^9'] * count
    if fixed or param_name in ["Appearance", "Spark Testing"]:
        return ['OK'] * count
    
    if param_name in pools:
        target_pool = pools[param_name]
        if target_pool:
            if len(target_pool) >= count:
                return random.sample(target_pool, count)
            else:
                res = []
                while len(res) < count:
                    res.append(random.choice(target_pool))
                return res
                
    if "±" in spec:
        try:
            base_str, var_str = spec.split("±")
            base = float(base_str.strip())
            var = float(var_str.replace("mm", "").strip())
            return [round(random.uniform(base-var, base+var), 3) for _ in range(count)]
        except:
            pass
    ...
    if m:
        ...
        return [round(random.uniform(low, high), 3) for _ in range(count)]
        
    if param_name == "Conductor resistance":
        return [round(random.uniform(34.0, 36.0), 2) for _ in range(count)]
    if param_name == "Insulation Thickness (Nom.)":
        return [round(random.uniform(0.39, 0.45), 2) for _ in range(count)]
    if param_name == "Insulator Elongation":
        return [random.randint(130, 160) for _ in range(count)]
    if param_name == "Conductor Elongation":
        return [round(random.uniform(14.0, 18.0), 1) for _ in range(count)]
    if param_name == "Tensile Strength":
        return [round(random.uniform(16.0, 20.0), 1) for _ in range(count)]
    if param_name == "Shrinkage":
        return [round(random.uniform(1.0, 3.5), 1) for _ in range(count)]
        
    return ['OK'] * count

parameters = [
    {"name": "No of strands", "spec": "20 nos.", "fixed": True, "cat": "A"},
    {"name": "Strands dia", "spec": "0.180 ± 0.008 mm", "fixed": False, "cat": "C"},
    {"name": "Insulation dia", "spec": "1.70-1.80 mm", "fixed": False, "cat": "C"},
    {"name": "Conductor resistance", "spec": "36.70 ohms/km Max.", "fixed": False, "cat": "B"},
    {"name": "Insulation Thickness (Nom.)", "spec": "0.35 mm", "fixed": False, "cat": "C"},
    {"name": "Temperature rating", "spec": "80°C", "fixed": True, "cat": "D"},
    {"name": "Voltage rating", "spec": "25 VAC, 60 VDC", "fixed": True, "cat": "D"},
    {"name": "Volume Resistivity", "spec": "10^9 ohm mm Min.", "fixed": True, "cat": "B"},
    {"name": "Withstand voltage", "spec": "1 kVAC for 1 min, no breakdown", "fixed": True, "cat": "B"},
    {"name": "Printing over the cable", "spec": "NA", "fixed": True, "cat": "A"},
    {"name": "Insulator Elongation", "spec": "125% Min.", "fixed": False, "cat": "B"},
    {"name": "Conductor Elongation", "spec": "13.5% Min.", "fixed": False, "cat": "B"},
    {"name": "Tensile Strength", "spec": "15.7 Mpa Min.", "fixed": False, "cat": "B"},
    {"name": "Shrinkage", "spec": "4% Max.", "fixed": False, "cat": "B"},
    {"name": "Spark Testing", "spec": "5 kVAC, no breakdown", "fixed": True, "cat": "B"},
    {"name": "Colour", "spec": "As per master sample", "fixed": False, "cat": "A"},
    {"name": "Appearance", "spec": "Should be visualy ok (No cut marks/burn marks/scratches etc)", "fixed": True, "cat": "A"}
]

try:
    ps_df = pd.read_excel(PACKING_SLIP_PATH)
except Exception as e:
    print(f"Error reading Packing Slip: {e}")
    exit(1)
...
def build_worksheet(ws, data):
    ...
    # Apply Borders from Row 3 to 35
    for r in range(3, 36):
        if r == 35:
            continue
        for c in range(1, 27):
...
for idx, row in ps_df.iterrows():
    ...
    if "INVOICE NO" in c2:
        ...
        if "Dt:" in c4:
            current_invoice_date = c4.replace("Dt:", "").strip()
    ...
    if c1.isdigit() and current_invoice_no:
        ...
        if pd.notna(length_val):
            lot_size_metres = f"{int(float(length_val))} m"
        
        print(f"Generating Excel for Invoice {current_invoice_no}, Item {item_code}, Coils: {num_coils}, Length: {lot_size_metres}")
        
        info = item_info_map.get(item_code, {})
        copper = info.get('copper_type', '')
        colour = info.get('colour', '')
        full_desc = info.get('group_name', '')
        
        try:
            inv_dt = datetime.strptime(current_invoice_date, "%d-%m-%Y")
        except:
            inv_dt = datetime.now()
        ...
        data = {
            ...
            'invoice_date': current_invoice_date.replace("-", "/"),
            'readings': readings_table
        }
        
        safe_item_code = item_code.replace("/", "_").replace("\\", "_")
        excel_path = os.path.join(OUTPUT_DIR, f"Inv_{current_invoice_no}_{safe_item_code}_v2.xlsx")
        
        # Individual Workbook
        ind_wb = Workbook()
        ind_ws = ind_wb.active
        build_worksheet(ind_ws, data)
        ind_wb.save(excel_path)
        
        # Master Workbook
        if master_ws_idx == 0:
            m_ws = master_wb.active
            m_ws.title = f"{safe_item_code[:30]}"
        else:
            m_ws = master_wb.create_sheet(title=f"{safe_item_code[:30]}_{master_ws_idx}")
            
        build_worksheet(m_ws, data)
        master_ws_idx += 1
            
master_wb.save(os.path.join(OUTPUT_DIR, "Master_All_Reports.xlsx"))
print("Done! Reports generated in Output_Reports_Excel directory.")
