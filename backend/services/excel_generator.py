...
# Use an environment variable or relative path in production
BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
ITEM_CODES_TRACKER_PATH = os.path.join(BASE_DIR, "sample_inputs", "Item Codes Tracker.xlsx")
TEST_READINGS_PATH = os.path.join(BASE_DIR, "sample_inputs", "TEST READINGS.xlsx")
...
def load_static_data():
    try:
        ...
        raise RuntimeError(f"Error reading Item Codes Tracker: {e}")
    ...
    try:
        ...
        raise RuntimeError(f"Error reading Test Readings: {e}")
    ...
    return item_info_map, pools

def generate_obs(param_name, fixed, copper_type, spec, count, pools, item_colour=""):
...
def process_packing_slip(packing_slip_path, output_dir):
    os.makedirs(output_dir, exist_ok=True)
    item_info_map, pools = load_static_data()
    
    try:
        ps_df = pd.read_excel(packing_slip_path)
    except Exception as e:
        raise RuntimeError(f"Error reading Packing Slip: {e}")
    ...
    for idx, row in ps_df.iterrows():
        ...
        if c1.isdigit() and current_invoice_no:
            ...
            readings_table = []
            for p in parameters:
                obs = generate_obs(p['name'], p['fixed'], copper, p['spec'], 8, pools, colour)
    ...
    return master_excel_path, master_ws_idx
